...
def normalize_label(label):
    ...
    return synonyms.get(label.lower(), label.lower().replace("_", " ").replace("-", " "))
...
async def upcycle(file: UploadFile = File(...), language: str = Form("en")):
    ...
    detected_objects = [normalize_label(labels.get(str(idx), ["Unknown","Unknown"])[1]) for idx in top_k_indices]
    ...
    if not recyclable_info:
        ...
        recycling_steps = ["Please check local recycling guidelines for proper disposal."]
    ...
    try:
        ...
        if language.lower() == 'hi':
            ...
            if recycling_steps:
                tts_text += " रिसायक्लिंग के चरण: " + ". ".join([step.replace("1.", "").replace("2.", "").replace("3.", "").replace("4.", "") for step in recycling_steps])
        else:
            tts_text = f"Detected object: {detected_obj if detected_obj else 'Unknown'}. {recyclable_info}"
            if recycling_steps:
                tts_text += " Recycling steps: " + ". ".join([step.replace("1.", "").replace("2.", "").replace("3.", "").replace("4.", "") for step in recycling_steps])
    ...
    return JSONResponse(content={
        "suggestions": "\n\n".join(suggestions) if suggestions else "No specific upcycling idea found.",
